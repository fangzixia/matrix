	...
	for c.scanner.Scan() {
		...
		if err := json.Unmarshal(line, &resp); err == nil && resp.ID != nil {
			...
			if ch, ok := c.pendingCalls.Load(normalizeRPCID(resp.ID)); ok {
				...
				default:
					logging.Warnf("Warning: response channel full for ID %v", resp.ID)
				}
			} else {
				logging.Warnf("Warning: received response for unknown ID %v", resp.ID)
			}
			continue
		}
		...
		if err := json.Unmarshal(line, &notif); err == nil && notif.Method != "" {
			...
			default:
				logging.Warnf("Warning: notification channel full, dropping notification: %s", notif.Method)
			}
			continue
		}
...
func (c *Client) logStderr() {
	...
	for scanner.Scan() {
		logging.Infof("[MCP stderr] %s", scanner.Text())
	}
}
