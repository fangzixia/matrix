...
func (r *Runtime) newMCPManager(extra []ports.MCPServerConfig) *mcp.Manager {
	...
	for name, s := range r.cfg.MCP.Servers {
		if s.Disabled {
			continue
		}
		if !r.cfg.AI.Security.AllowCommandMCP && s.Command != "" {
			continue
		}
	...
	for _, e := range extra {
		...
		if !r.cfg.AI.Security.AllowCommandMCP && e.Command != "" {
			continue
		}
...
func (r *Runtime) buildQueryConfig(
	...
	ctxPolicy := query.ContextPolicy{
		AutoCompactThreshold: r.cfg.AI.Context.AutoCompactThreshold,
		KeepRecentMessages:   r.cfg.AI.Context.KeepRecentMessages,
	}
