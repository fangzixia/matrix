...
import { Alert, Button, Checkbox, Form, Input, Space, Table, Tabs } from 'antd'
...
export default function SettingsRepositoriesPage() {
  ...
  async function add(values: { name: string; git_url: string; git_branch: string; is_default: boolean }) {
    setError('')
    try {
  ...
  return (
    <div>
      <Tabs
        ...
        items={settingsTabs(projectId).map((tab) => ({ key: tab.key, label: tab.label }))}
        style={{ marginBottom: 16 }}
      />
      <h2>仓库</h2>
      {error && <Alert type="error" message={error} style={{ marginBottom: 16 }} />}
      {message && <Alert type="success" message={message} style={{ marginBottom: 16 }} />}
      <Form form={form} className="panel stack" layout="vertical" style={{ maxWidth: 560, marginBottom: 24 }} initialValues={{ git_branch: 'main', is_default: false }} onFinish={add}>
        <Form.Item label="名称" name="name" rules={[{ required: true }]}><Input /></Form.Item>
        <Form.Item label="Git 地址" name="git_url"><Input /></Form.Item>
        <Form.Item label="分支" name="git_branch"><Input /></Form.Item>
        <Form.Item name="is_default" valuePropName="checked"><Checkbox>设为默认仓库</Checkbox></Form.Item>
        <Button type="primary" htmlType="submit">添加仓库</Button>
      </Form>
      <Table dataSource={repos} rowKey="id" pagination={false}>
        ...
        <Table.Column title="默认" render={(_, row: Repository) => row.is_default ? '是' : ''} />
        <Table.Column title="操作" render={(_, row: Repository) => (
          <Space>
