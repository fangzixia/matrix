  ...
  return (
    <div>
      <div className="flex-between">
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/admin/users/new')}>新建用户</Button>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        <Input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="搜索用户…" style={{ width: 320 }} />
        <span className="muted">共 {filtered.length} 位用户</span>
      </div>
      <Table dataSource={filtered} rowKey="id" pagination={{ pageSize: 20 }}>
        <Table.Column title="" width={48} render={(_, row: UserWithStats) => (
        ...
        <Table.Column title="状态" render={(_, row: UserWithStats) => (
          <>
            <Tag color={row.state === 'active' ? 'success' : 'error'}>{row.state}</Tag>
            {row.is_admin && <Tag color="warning">管理员</Tag>}
          </>
        )} />
        <Table.Column title="上次登录" render={(_, row: UserWithStats) => (
          row.last_sign_in_at ? new Date(row.last_sign_in_at).toLocaleString() : '—'
        )} />
        <Table.Column title="操作" render={(_, row: UserWithStats) => (
          <Space>
            <Button type="link" onClick={() => navigate(`/admin/users/${row.id}`)}>编辑</Button>
            <Button type="link" onClick={() => setResetTarget(row)}>重置密码</Button>
            <Button type="link" onClick={() => toggleBlock(row)}>
              {row.state === 'active' ? '封禁' : '解封'}
            </Button>
            <Button type="link" danger onClick={() => setDeleteTarget(row)}>删除</Button>
          </Space>
        )} />
      </Table>
      <Modal open={!!deleteTarget} title="删除用户" onCancel={() => setDeleteTarget(null)} onOk={confirmDelete} okText="删除" okButtonProps={{ danger: true }}>
        确定删除用户 <strong>{deleteTarget?.username}</strong>？
      </Modal>
      <Modal open={!!resetTarget} title="重置密码" onCancel={() => setResetTarget(null)} onOk={confirmReset} okText="重置" okButtonProps={{ disabled: !newPassword }}>
