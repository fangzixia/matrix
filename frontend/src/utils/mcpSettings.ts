...
export function parseMcpServersJson(
  ...
  if (
    obj.mcp_servers &&
    typeof obj.mcp_servers === "object" &&
    !Array.isArray(obj.mcp_servers)
  ) {
...
function normalizeMcpServers(
  ...
  for (const [name, val] of Object.entries(raw)) {
    ...
    if (Array.isArray(s.args))
      entry.args = s.args.filter((a): a is string => typeof a === "string");
    ...
    if (
      ...
      entry.headers = Object.fromEntries(
        Object.entries(s.headers as Record<string, unknown>).filter(
          ([, v]) => typeof v === "string",
        ),
