...
// formatTranscriptForSummary 将历史消息格式化为摘要输入文本。
...
func llmCompactHistory(ctx context.Context, cfg Config, msgs []Message, budget int, kind string) ([]Message, bool) {
	...
	if err != nil {
		agentLog(ctx, "query: LLM autoCompact 失败，将回退确定性压缩", "error", err.Error())
		return msgs, false
	}
	...
	slog.Info("query: LLM autoCompact 完成",
		"kind", kind,
...
// shouldProactiveAutoCompact 判断是否应主动触发 LLM 全量压缩。
