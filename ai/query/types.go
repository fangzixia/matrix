...
type Config struct {
	...
	LogPrefix string
	// ThreadID 为 AG-UI threadId（宿主注入，如 projectId / chatSessionId）。
	ThreadID string
	// RunID 为 AG-UI runId（单次 RunSession）；空则由 RunSession 生成。
	RunID string
	// ParentRunID 为可选的上层 runId（如 Matrix Job 或上一阶段）。
	ParentRunID string
	// SessionID 为审计/日志关联 ID；空则与 RunID 相同。
	SessionID string
	// Audit 为可选会话诊断写入器；nil 时不记录诊断事件。
	Audit AuditRecorder
}
...
// StreamSink 为 RunSession 的过程消息出口（见 matrix/ai/stream）。
