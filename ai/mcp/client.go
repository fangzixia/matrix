...
func (c *Client) Initialize() error {
	...
	c.initialized = true
	c.mu.Unlock()
	slog.Info("MCP client initialized", "server", result.ServerInfo.Name, "version", result.ServerInfo.Version)
	return nil
}
	...
	for c.scanner.Scan() {
		...
		if err := json.Unmarshal(line, &resp); err == nil && resp.ID != nil {
			...
			if ch, ok := c.pendingCalls.Load(normalizeRPCID(resp.ID)); ok {
				...
				default:
					slog.Warn("MCP: response channel full", "id", resp.ID)
				}
			} else {
				slog.Warn("MCP: received response for unknown ID", "id", resp.ID)
			}
			continue
		}
		...
		slog.Warn("MCP: received unknown message", "line", string(line))
	}
	if err := c.scanner.Err(); err != nil {
		slog.Error("MCP: error reading stdout", "error", err)
	}
}
...
func (c *Client) logStderr() {
	...
	for scanner.Scan() {
		slog.Info("MCP stderr", "line", scanner.Text())
	}
}
