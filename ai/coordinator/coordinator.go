...
type Config struct {
	...
	ToolRegistry *util.Registry
	// CanUseTool 为 Worker 的权限检查回调；nil 表示允许所有工具。
	CanUseTool util.CanUseToolFn
	...
	StreamHub *StreamHub
	// ThreadID 为 AG-UI threadId，Worker RunSession 继承。
	ThreadID string
	// RunID 为父 RunSession 的 AG-UI runId；Worker 使用独立 runId 并设 ParentRunID。
	RunID string
	// SessionID 与父会话流式 channel 对齐（Worker RunSession 用）。
	SessionID string
	// Audit 会话诊断写入器（与父 query.Config.Audit 共享）。
	Audit query.AuditRecorder
	// SandboxDir 为宿主工作区根目录，仅用于 WorkerUserMessageFormat 等宿主编排（非 tools 沙箱定义）。
	SandboxDir string
	// WorkerUserMessageFormat 格式化 Worker 首条 user 消息（由宿主注入目录上下文）。
	WorkerUserMessageFormat func(prompt string) string
}
...
//
// 标记为并发安全：[util.RunTools] 将同一批次的多个 agent 调用并行执行。
func NewAgentTool(cfg Config) *util.Tool {
	return &util.Tool{
		Name:        "agent",
		Description: "派生一个 Worker Agent 执行子任务。默认在配置了异步支持时立即返回 ACK 并在后台运行；未配置时同步阻塞直至完成，结果均为 <result> XML。",
		Parameters: util.JSONSchema{
			Type: "object",
			Properties: map[string]util.PropSchema{
				"description": {
					Type:        "string",
		...
		StatusLabel: func(args map[string]any) string {
			description, _ := util.GetString(args, "description")
			if description == "" {
				return ""
			}
			return fmt.Sprintf("Worker: %s …", description)
		},
...
func makeAgentExecute(cfg Config) func(context.Context, map[string]any) (string, error) {
	return func(ctx context.Context, args map[string]any) (string, error) {
		description, _ := util.GetString(args, "description")
		prompt, _ := util.GetString(args, "prompt")
		sysPrompt, _ := util.GetString(args, "system_prompt")
		if sysPrompt == "" {
		...
		parentToolUseID := util.ToolCallIDFromContext(ctx)
		id := agent.NewID()
		rec := &agent.Record{
			ID:              id,
		...
		if cfg.Async != nil {
			cfg.Async.Inc()
			go func() {
				slog.Info("Worker 启动", "agent_id", id, "description", description)
		...
		slog.Info("Worker 启动（同步）", "agent_id", id, "description", description)
		result := runSubAgentWorker(cfg, id, subCfg, workerSink)
		finishSubAgent(cfg, id, result)
		slog.Info("Worker 完成（同步）", "agent_id", id, "turns", result.TurnCount)
...
func buildWorkerConfig(
	...
	if cfg.WorkerUserMessageFormat != nil {
		content = cfg.WorkerUserMessageFormat(prompt)
	}
	qc := QueryConfigFromCoordinator(cfg, QueryConfigOverrides{
		...
		InitialMessages: []query.Message{
			{Role: query.RoleUser, Content: content},
		},
	})
	qc.RunID = stream.NewRunID()
	qc.ParentRunID = cfg.RunID
	return qc
}
...
func NewSendMessageTool(cfg Config) *util.Tool {
	return &util.Tool{
		Name:        "send_message",
...
func makeSendMessageExecute(cfg Config) func(context.Context, map[string]any) (string, error) {
	return func(ctx context.Context, args map[string]any) (string, error) {
		to, _ := util.GetString(args, "to")
		message, _ := util.GetString(args, "message")
		agentID := agent.ID(to)
		...
		slog.Info("coordinator: 续接完成", "id", agentID, "turns", result.TurnCount, "stop", result.StopReason)
...
func NewTaskStopTool(cfg Config) *util.Tool {
	return &util.Tool{
		Name:        "task_stop",
		...
		Execute: func(ctx context.Context, args map[string]any) (string, error) {
			taskID, _ := util.GetString(args, "task_id")
			reason, _ := util.GetString(args, "reason")
			if reason == "" {
