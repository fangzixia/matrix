...
func NewParentRegistry(cfg Config) *util.Registry {
	reg := util.NewRegistry()
	RegisterTools(reg, cfg)
	reg.SetUnknownToolHint(func(name string) string {
		if IsWorkerBuiltinTool(name) {
			return WorkerOnlyToolMessage(name)
		}
		return ""
	})
	return reg
