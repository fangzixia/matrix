...
4. 建议调用前先用 read_file 读取文件，确认 old_string 内容准确。`,
		...
		StatusLabel: func(args map[string]any) string {
			...
			return fmt.Sprintf("编辑 %s …", filePath)
		},
...
func execFileEdit(ctx context.Context, args map[string]any) (string, error) {
	filePath, _ := getString(args, "file_path")
	oldStr, _ := getString(args, "old_string")
	newStr, _ := getString(args, "new_string")
	replaceAll, _ := args["replace_all"].(bool)
	resolvedPath, resolveErr := access.ResolveAllowed(ctx, filePath)
	if resolveErr != nil {
