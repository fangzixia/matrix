...
var ReadFile = &Tool{
	Name:        "read_file",
	Description: "读取允许目录内指定路径的文件内容（文本）。",
	Parameters: JSONSchema{
		Type: "object",
		Properties: map[string]PropSchema{
			"path": {Type: "string", Description: access.ReadPathParamDesc},
		},
	...
	Execute: func(ctx context.Context, args map[string]any) (string, error) {
		...
		targetPath, resolveErr := access.ResolveAllowed(ctx, path)
		if resolveErr != nil {
...
var WriteFile = &Tool{
	Name:        "write_file",
	Description: "将文本内容写入允许目录内指定路径的文件（覆盖写入）。",
	Parameters: JSONSchema{
		Type: "object",
		Properties: map[string]PropSchema{
			"path":    {Type: "string", Description: access.PathParamDesc},
	...
	Execute: func(ctx context.Context, args map[string]any) (string, error) {
		...
		util.WriteString(ctx, msg+"\n")
		return msg, nil
	},
}

// ListDir 列出允许目录内指定目录下的文件与子目录。
var ListDir = &Tool{
	Name:        "list_dir",
	Description: "列出允许目录内指定目录下的条目；path 须为绝对路径。",
	Parameters: JSONSchema{
		Type: "object",
		Properties: map[string]PropSchema{
			"path": {Type: "string", Description: access.PathParamDesc},
...
var PowerShell = &Tool{
	...
	StatusLabel: func(args map[string]any) string {
		...
		return fmt.Sprintf("PS> %s", command)
	},
